...
fspecs = [('lastname', str, 8), ('age', int, 4)]
    ...
    print kind_line.format(f_id=f_id)
    for fname, fkind, fwidth in fspecs:
        if fkind == str:
            print str_line.format(fname=fname, fkind=fkind, fwidth=fwidth)
        else:
            print other_line.format(fname=fname, fkind=fkind, fwidth=fwidth)
