...
data = map(Int, [10, 20, 30, 50, 20, 5, 10, 15, 20])
a = Int(111)
b = Int(20)
c = data[2]

odata = map(Int, [20, 30, 40, 300, 400, 500, 300, 400]) + data[-4:]
u = set(odata)
...
print 'Fundamental rule of sets:  Using sets is *much* cheaper than building them'
reset(); print s & u; show()            # Cost of intersection/union/difference: 0 hashes and 1 compare per non-identity overlap
reset(); print s | u; show()
reset(); print s - u; show()
reset(); print dict.fromkeys(s); show() # Cost of conversion to a dict is 0 hashes and 0 compares
